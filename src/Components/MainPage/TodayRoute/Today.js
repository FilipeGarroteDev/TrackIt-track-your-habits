    ...
    <>
      <TitleContainer checkedHabits={todaysHabits.filter(habit => habit.done)}>
        ...
        {todaysHabits.filter(habit =>
          habit.done === true).length === 0 ?
            <h3>Nenhum hábito concluído ainda</h3>
          :
            <h3>{`${(Math.round(checkedHabits/totalHabits*100))}% dos hábitos concluídos`}</h3>}
      </TitleContainer>
...
const TitleContainer = styled.div`
  ...
  h3{
    font-size: 18px;
    color: ${props => props.checkedHabits.length === 0 ? props.theme.fontColor.subtitles : "#8FC549"};
    margin-top: 4px;
  }
  `;